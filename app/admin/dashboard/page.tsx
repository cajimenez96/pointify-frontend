  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Monitoreo en tiempo real de tu campaña de lealtad</p>
      </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Estado de Campaña</h3>
            <p className="text-sm text-gray-600 mt-1">{settings?.rewardName}</p>
          </div>
          <Badge variant={settings?.isActive ? 'default' : 'secondary'} className="text-lg px-4 py-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Stock de Premios */}
        <Card className="p-6">
          <h3 className="text-sm font-medium text-gray-600 mb-2">Stock de Premios</h3>
          {settings?.maxWinners === 0 ? (
            <div>
              <p className="text-3xl font-bold text-gray-900">{settings?.currentWinners}</p>
            ...
            <div>
              <div className="flex items-baseline gap-2">
                <p className="text-3xl font-bold text-gray-900">{settings?.currentWinners}</p>
                <p className="text-xl text-gray-500">/ {settings?.maxWinners}</p>
              </div>
              <Progress value={stockPercentage} className="mt-3" />
              <p className="text-sm text-gray-600 mt-2">
                {stockPercentage >= 100 ? (
                  <span className="text-red-600 font-semibold">⚠️ Stock agotado</span>
                ) : stockPercentage >= 80 ? (
                  <span className="text-orange-600 font-semibold">⚠️ Stock bajo</span>
                ) : (
        ...
        <Card className="p-6">
          <h3 className="text-sm font-medium text-gray-600 mb-2">Conversión de Usuarios</h3>
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-gray-600 mt-3">
            Promedio: {stats?.totalTransactions && clients.length > 0
        ...
        {/* Puntos Emitidos */}
        <Card className="p-6">
          <h3 className="text-sm font-medium text-gray-600 mb-2">Puntos Emitidos</h3>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-4xl font-bold text-blue-600 mt-2">{clients.length}</p>
          </div>
          ...
          <div className="text-center p-4 bg-yellow-50 rounded-lg">
            ...
            <p className="text-4xl font-bold text-yellow-600 mt-2">{shadowClients}</p>
          </div>
        </div>
      </Card>
    </div>
