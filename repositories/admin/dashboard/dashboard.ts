...
import { DashboardStats, ClientSummary, DashboardError, ApiError } from './types';
...
export async function getClients(): Promise<ClientSummary[]> {
  try {
    const { data } = await apiClient.get<ClientSummary[]>('/clients');
    return data;
  } catch (error: any) {
    if (error.response) {
      const apiError = error.response.data as ApiError;
      throw new DashboardError(
        Array.isArray(apiError.message)
          ? apiError.message.join(', ')
          : apiError.message || 'Error al obtener clientes',
        error.response.status,
        apiError
      );
    }
